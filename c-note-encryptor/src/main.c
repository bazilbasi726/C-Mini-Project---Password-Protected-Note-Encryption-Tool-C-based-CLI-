...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(mode_flag, "-e") == 0) {
        ...
        print_usage(program_name);
        log_head = loglist_append(log_head, "Invalid or missing mode flag.");
        return STATUS_INVALID_ARGUMENT;
    }
    ...
    if (strcmp(info.input_path, info.output_path) == 0) {
        fprintf(stderr, "Input and output paths cannot match.\n");
        log_head = loglist_append(log_head, "Input/output paths were identical.");
    ...
    if (!fgets(password, sizeof(password), stdin)) {
        ...
        log_head = loglist_append(log_head, "Password read failed.");
        loglist_print(log_head);
        loglist_free(log_head);
    ...
    if (password[0] == '\0') {
        ...
        log_head = loglist_append(log_head, "Password was empty.");
        loglist_print(log_head);
        loglist_free(log_head);
        return STATUS_PASSWORD_ERROR;
        return STATUS_PASSWORD_ERROR;
    ...
    log_head = loglist_append(log_head, info.mode == MODE_ENCRYPT ? "Mode set to encrypt." : "Mode set to decrypt.");
